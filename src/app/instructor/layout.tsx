...
export default function InstructorLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    { name: 'Dashboard', icon: <LayoutDashboard className="w-5 h-5" />, path: '/instructor/dashboard' },
    { name: 'Proposal Vetting', icon: <ClipboardList className="w-5 h-5" />, path: '/instructor/vetting' },
    { name: 'Roster Allocation', icon: <Users className="w-5 h-5" />, path: '/instructor/allocation' },
    { name: 'Messages', icon: <MessageSquare className="w-5 h-5" />, path: '/messages' },
    { name: 'Settings', icon: <Settings className="w-5 h-5" />, path: '/instructor/settings' },
  ]
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 flex overflow-hidden font-sans">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-emerald-100/60 blur-[120px] rounded-full" />
      ...
      >
        <div className="h-20 flex items-center px-6 border-b border-slate-200">
          <Link href="/" className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-br from-emerald-600 to-teal-700 rounded-lg flex items-center justify-center shrink-0">
        ...
        <div className="p-4 border-t border-slate-200">
          <div className="bg-slate-50 rounded-2xl p-3 flex items-center gap-3 overflow-hidden border border-slate-200">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-600 to-teal-700 flex items-center justify-center font-bold text-lg shrink-0 text-white">
              {userName[0]}
            </div>
            {isSidebarOpen && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 min-w-0">
                <div className="font-bold text-sm truncate text-slate-900">{userName}</div>
                <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest">Instructor</div>
              </motion.div>
            )}
            {isSidebarOpen && (
        ...
        <header className="h-20 flex items-center justify-between px-8 bg-white/80 backdrop-blur-md border-b border-slate-200 shrink-0 z-40">
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden md:block">
              ...
              <input placeholder="Search projects..." className="bg-slate-50 border border-slate-200 rounded-xl py-2 pl-10 pr-4 text-sm w-64 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/40 focus:border-emerald-400" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 relative">
            ...
            >
              ...
              {unreadCount > 0 && (
                <div className="absolute top-2 right-2 w-2 h-2 bg-emerald-600 rounded-full border-2 border-white" />
              )}
            </button>
              ...
              {showNotifications && (
                ...
                >
                  <div className="flex items-center justify-between mb-4 px-2">
                    ...
                    <button className="text-[10px] text-emerald-600 font-bold hover:underline">Mark all as read</button>
                  </div>
                      ...
                      <div key={n.id} className={`p-3 rounded-2xl border transition-all ${n.is_read ? 'bg-transparent border-transparent' : 'bg-slate-50 border-slate-200'}`}>
                        <div className="flex gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                            n.type === 'deadline' ? 'bg-red-50 text-red-500' : 'bg-emerald-50 text-emerald-600'
                          }`}>
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                <div className="text-xs font-bold text-slate-900">Cohort Health</div>
                <div className="text-[10px] text-emerald-600 font-bold">Stable</div>
              </div>
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-600 to-teal-700 flex items-center justify-center font-bold text-white">
                {userName[0]}
              </div>
            </div>
          </div>
