...
export default function ExaminerDashboardPage() {
  ...
  useEffect(() => {
    async function initPanelDashboard() {
      ...
      if (!prof || (prof.role !== 'examiner' && prof.role !== 'examiner_panel')) {
  ...
  async function fetchDashboardData(userId?: string, userProf?: any) {
    ...
    // Filter to keep ONLY projects where this examiner is explicitly assigned by admin
  ...
  async function handleSubmitEvaluation(e: React.FormEvent) {
    ...
    try {
      ...
      if (notifError) console.warn("Notification write failed:", notifError)
      ...
      if (user) {
        ...
        if (msgError) console.warn("Message write failed:", msgError)
      }
    } catch (err: any) {
      ...
      <div className="space-y-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-slate-400 mt-1">
                {profile.full_name || 'Panel Examiner'} - Senior Capstone Evaluator {profile.department ? `(${profile.department} Department)` : ''}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex border-b border-slate-200 overflow-x-auto gap-6 no-scrollbar">
          {[
            ...
            { id: 'projects', label: 'Assigned Projects', icon: <Clock className="w-4 h-4" /> },
          ...
          >
            ...
            {activeTab === 'overview' && (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  {/* Card 1: Review Capstone Projects */}
                  <div 
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-emerald-100 mt-1.5 font-bold uppercase tracking-wider">Assigned projects</p>
                    </div>
                  </div>
                ...
                <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm space-y-6">
                  <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                    <h3 className="text-lg font-black text-slate-900 tracking-tight">Recent Assigned Submissions</h3>
                  </div>
            ...
            {activeTab === 'projects' && (
              <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm space-y-6">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Assigned Capstone Projects</h3>
                  <p className="text-xs text-slate-500 font-semibold mt-1">Audit assigned research papers, student deliverables, and advisor alignments.</p>
                </div>
                {projects.length > 0 ? (
                  ...
                  <div className="py-12 text-center text-slate-400 font-bold text-xs uppercase tracking-wider">
                    No projects allocated to your panel by the admin yet.
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'evaluations' && (
              <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm space-y-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-semibold mt-1">Submit presentation review feedback, raise vetting questions, and track capstone deliveries.</p>
                </div>
                {projects.length > 0 ? (
            ...
            {activeTab === 'presentations' && (
              <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm space-y-6">
                ...
                {projects.length > 0 ? (
                  ...
                  <div className="py-12 text-center text-slate-400 font-bold text-xs uppercase tracking-wider">
                    No upcoming defense presentations scheduled for assigned projects.
                  </div>
                )}
              </div>
            )}
