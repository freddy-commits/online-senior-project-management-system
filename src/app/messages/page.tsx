...
export default function MessagesPage() {
  ...
  return (
    <DashboardLayout role={userProfile.role} userName={userProfile.full_name}>
      <div className="h-[calc(100vh-160px)] flex bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm">
          ...
          <div className="p-6 border-b border-slate-200">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 pl-10 pr-4 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all text-slate-900 placeholder:text-slate-500"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {contacts.map((contact) => (
              <button 
                ...
                className={`w-full p-4 flex items-center gap-4 hover:bg-slate-50 transition-all border-b border-slate-100 cursor-pointer ${
                  selectedContact?.id === contact.id ? 'bg-blue-50 border-l-4 border-l-blue-600' : ''
                }`}
              >
                <div className="w-12 h-12 rounded-2xl bg-blue-100 flex items-center justify-center font-bold text-blue-600 shrink-0">
                  {contact.full_name[0]}
                </div>
                <div className="text-left overflow-hidden">
                  <div className="font-bold text-sm text-slate-900 truncate">{contact.full_name}</div>
        ...
        <div className="flex-1 flex flex-col relative">
          {selectedContact ? (
            <>
              {/* Chat Header */}
              <div className="h-20 flex items-center justify-between px-8 border-b border-slate-200 bg-slate-50/50">
              ...
              <div className="p-6 bg-white border-t border-slate-200">
                <form onSubmit={handleSendMessage} className="flex gap-4">
                  <input 
                    ...
                    className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all text-slate-900 placeholder:text-slate-500"
                  />
                  <button 
                    type="submit"
                    className="w-14 h-14 bg-blue-600 hover:bg-blue-500 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/20 transition-all active:scale-[0.9] shrink-0 cursor-pointer"
                  >
